  ...
  <div id="editor">
    ...
    <ol class="panes">
      <li v-bind:class="{active:currentTab===0}">
        <ProfileEditor v-bind:items="profile" v-bind:labels="{name:'姓名',city:'工作内容',birthday:'出生年月'}"
      ...
      <li v-bind:class="{active:currentTab===3}">
        <ArrayEditor v-bind:items="projects" v-bind:labels="{name:'项目名称',content:'内容'}" title="项目经历"></ArrayEditor>
      </li>
      <li v-bind:class="{active:currentTab===4}">
        <ArrayEditor v-bind:items="awards" v-bind:labels="{name:'奖励详情'}" title="获奖情况"></ArrayEditor>
      </li>
      <li v-bind:class="{active:currentTab===5}">
        <ProfileEditor v-bind:items="contacts" v-bind:labels="{qq:'QQ',wechat:'微信',email:'邮箱',phone:'手机'}"
